...
:mutinyapi: https://javadoc.io/doc/io.smallrye.reactive/mutiny/2.1.0
...
See the xref:negotiation[Negotiation] section for more information.
...
See xref:parameter-mapping[Parameter mapping] for more advanced use-cases.
...
|Advanced: Access to the current HTTP method and xref:preconditions[Preconditions]
...
Such a response filter will also be called for xref:exception-mapping[handled] exceptions.
...
|Any thrown and xref:exception-mapping[handled] exception, or `null` (only for response filters).
...
[NOTE]
====
A `ContainerResponseFilter` will also be called for xref:exception-mapping[handled] exceptions.
====
