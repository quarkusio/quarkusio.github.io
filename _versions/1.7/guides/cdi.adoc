...
== _Q: Wait a minute. What does "container-managed" mean?_
...
== _Q: What does a bean look like?_
...
<3> This is an interceptor binding annotation. In this case, the annotation comes from the MicroProfile Metrics. The relevant interceptor intercepts the invocation and updates the relevant metrics. We will about <<interceptors,interceptors>> later.
...
NOTE: Synthetic beans are usually provided by extensions. Therefore, we not going to cover them in this guide.
