...
:categories: data,execution-model
:categories-path: data.data-datasources,execution-model.execution-model-reactive
...
You can reference a named TLS configuration using the `tls-configuration-name` property:
...
When a named TLS configuration is set, SSL is automatically enabled for the connection.
For databases that support SSL modes (PostgreSQL and MariaDB/MySQL), the SSL mode is set to `require`/`required`.
For IBM Db2 and Microsoft SQL Server, the `ssl` flag is set to `true`.

NOTE: When a named TLS configuration is set, manual SSL properties (`trust-certificate-*`, `key-certificate-*`, `trust-all`, `hostname-verification-algorithm`) are ignored.
