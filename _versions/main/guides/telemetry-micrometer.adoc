...
Tags can be specified when a meter is registered with a `MeterRegistry` or using a xref:meter-filter[Meter Filter].
...
<2> Associate xref:define-tags[tags] with the gauge. 
...
<4> Associate xref:define-tags[tags] with the gauge
...
<4> Associate xref:define-tags[tags] with the counter
