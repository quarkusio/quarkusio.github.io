...
:categories: cloud
:categories-path: cloud.cloud-functions
...
NOTE: For Gradle projects please <<gradle,see below>>, or for further reference consult the guide in the xref:gradle-tooling.adoc[Gradle setup page].
...
* `function.zip` - lambda deployment file (produced when using the default `legacy-jar` packaging)
...
By default, `quarkus-amazon-lambda` uses `quarkus.package.jar.type=legacy-jar` so that `function.zip` matches the layout expected by ZIP/SAM deployments.
For JVM container images that need the fast-jar layout (`target/quarkus-app/`) and `Dockerfile.jvm`, set `quarkus.package.jar.type=fast-jar`.
In that case `function.zip` is not generated.
...
NOTE: Quarkus now has extensions for DynamoDB, S3, SNS and SQS (more coming).
...
For a regular JVM distribution you need to base your image off the official AWS Java base images. Below is an example of a Dockerfile that would create a container image from your Quarkus Lambda project. It assumes that `mvn package` has been executed and binaries are available in the `target/` directory.

The default packaging is `legacy-jar`, which produces a runner jar plus a `lib/` directory:
...
To use fast-jar instead (for the `quarkus-app` layout, or so the container-image extension selects `Dockerfile.jvm`), set:
...
Then package with `mvn package` (or enable the container-image extension with `-Dquarkus.container-image.build=true`).
The build will succeed without requiring `legacy-jar`, and `function.zip` will not be produced.
If you deploy with the official AWS Lambda Java base image, adapt the `COPY` paths to the `target/quarkus-app/` layout.

=== Native executable container image

To create a lambda container image that uses the native executable we'll need to do things a little differently. In this case, we won't need to use the AWS Java base image, but instead we'll use a special image that assumes that the runtime environment for the lambda is provided. The example below creates such a container. It assumes that a Maven build has been executed (such as `mvn package -Dnative=true`) and has generated the native binary into the `target/` directory. The binary needs to be named `bootstrap` and be placed in `/var/runtime/`:
...
Below, you can see how the container images created above can be built and deployed to AWS using the `docker` and `aws` command line tools. These instructions work for both native and jvm container images and assume that the `aws` command line tool has been logged in.
