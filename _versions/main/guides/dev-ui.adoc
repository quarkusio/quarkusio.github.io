...
====== Qomponent
https://github.com/qomponent[Qomponent]: A few custom build components that can be used. 
...
This component uses the above mentioned code block from qomponent, that is built with https://codemirror.net/[code-mirror], but adds the automatic theme state when switching themes.
...
All communication to the backend happens with https://www.jsonrpc.org/[JsonRPC] over web socket. Dev UI makes this easy to use for extension developers, and you do not really need to get into the details of either JsonRPC or Web Sockets.
