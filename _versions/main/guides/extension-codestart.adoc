...
.Maven
****
Quarkus provides the xref:building-my-first-extension.adoc#maven-setup[`create-extension` Maven Mojo] to initialize an extension project.
...
* (Optional) Move the config using this convention: xref:app-config[application config application.yml].

* Create a xref:codestart-yml[codestart.yml] file in `runtime/src/main/codestarts/quarkus/aloha-codestart`:
+
[source,yaml]
----
...
* Add the readme xref:readme-md[README.md] section template in `base/README.tpl.qute.md`:
+
[source,html]
----
...
* If the extension provides some web resources, add the `base/src/main/resources/META-INF/resources/index.entry.qute.html` template (xref:index-html[index.html and web extension codestarts]).
...
The extension provides helpers to test that the projects build `buildAllProjects` or just a specific language project `buildProject(Language language)`. It also provides helpers to test the content with xref:snapshot-testing[Snapshot testing].
