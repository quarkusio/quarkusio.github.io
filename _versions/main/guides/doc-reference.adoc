...
<2> Define the document title following guidance in xref:titles-headings[Titles and headings].
<3> Include common document attributes.
<4> Specify the diataxis type: `concept`, `howto`, `reference`, or `tutorial`.
<5> Specify the relevant xref:categories[Categories] (comma separated).
...
The value of this attribute is used in tiles or other descriptions on the website and is not required in newer diataxis-styled docs, as outlined in xref:abstracts-preamble[Abstracts (preamble)].
...
Tag your content to improve findability by adding at least one category to the categories attribute line in the xref:document-header[document header]. To add multiple categories, use comma-separated values. For example:
