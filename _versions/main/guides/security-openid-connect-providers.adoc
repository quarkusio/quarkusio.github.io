...
For example, set `Category` to `SocialMotivation`, and set `ApplicationCallbackDomain` to either `localhost` or the domain name provided by Ngrok, see the xref:redirect_url[HTTPS Redirect URI] for more information.
...
Most OIDC and OAuth2 providers with the exception of xref:strava[Strava] will enforce that the authorization code flow can be completed only if the redirect URI matches precisely the redirect URI configured in a given provider's dashboard.
