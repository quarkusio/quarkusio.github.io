...
Migrate an application from xref:opentracing.adoc[OpenTracing] to xref:opentelemetry.adoc[OpenTelemetry tracing] in Quarkus 3.x.
...
Let's say instead of the `GreetingResource` class from above, you have something more complex. You will need additional work on top of the changes from the xref:starting-point[Starting point].
...
To proceed with this section, the code project must be its xref:starting-point[Starting point]. If you have changes related to the previous sections, please revert them or re-generate the project according to the xref:starting-point[Starting point] instructions before proceeding.
...
Remembering the initial version of the `GreetingResource` class from the  xref:greeting-resource-starting-point[The GreetingsResource with OpenTracing manual instrumentation]:
[source, java]
----
package org.acme;
