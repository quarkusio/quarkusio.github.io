...
You can trigger a build and deployment in a single step or build the container image first and then configure the OpenShift application manually if you need xref:control_application_config[more control over the deployment configuration].
...
TIP: If you want to test your application immediately then set the `quarkus.openshift.route.expose` config property to `true` to xref:exposing_routes[expose the service automatically], e.g. add `-Dquarkus.openshift.route.expose=true` to the command above.
