...
See xref:customizing-the-vert-x-configuration[Customize the Vert.x configuration] to configure the Vert.x instance using a programmatic approach.
...
NOTE: To use `io_uring`, refer to the xref:use-io_uring[Use io_uring] section.
...
IMPORTANT: This will only work on platforms that support xref:native-transport[Use native transports].

Enable the appropriate xref:native-transport[Use native transports] and set the following environment property:
