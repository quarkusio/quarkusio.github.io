...
If you need to enable or disable the exporter at runtime, you can use the xref:sampler[Sampler] because it has the ability to filter out all the spans if needed.
...
To use the built-in sampler, you can configure it by setting the desired sampler parameters as detailed in the xref:configuration-reference[OpenTelemetry Configuration Reference]. As an example, you can configure the sampler to retain 50% of the traces:
