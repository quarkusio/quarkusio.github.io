...
[.description]
--
The container image name to use, for container based DevServices providers. Image with a Quarkus based Keycloak-X distribution is used by default. Image with a WildFly based Keycloak distribution can be selected instead, for example: 'quay.io/keycloak/keycloak:16.0.0'. 
...
--|string 
|`quay.io/keycloak/keycloak-x:16.0.0`
...
a|icon:lock[title=Fixed at build time] [[quarkus-oidc_quarkus.oidc.devui.web-clien-timeout]]`link:#quarkus-oidc_quarkus.oidc.devui.web-clien-timeout[quarkus.oidc.devui.web-clien-timeout]`
