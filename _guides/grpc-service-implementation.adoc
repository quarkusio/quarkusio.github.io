...
|Username and password |xref:basic-auth-mechanism[Basic authentication]

|Client certificate |xref:mutual-tls-auth-mechanism[Mutual TLS authentication]

|Custom requirements |xref:custom-auth-mechanism[Custom authentication]
...
TIP: If you use separate HTTP server to serve gRPC requests, xref:custom-auth-mechanism[Custom authentication] is your only option.
...
The simplest way to enforce authentication for all your gRPC services is described in the xref:tls-with-mutual-auth[TLS with Mutual Auth] section of this guide.
