...
1990's. We won't go into all of the details behind STM but the interested reader could look at https://groups.csail.mit.edu/tds/papers/Shavit/ShavitTouitou-podc95.pdf[this paper].
...
Note, the STM implementation provided by Quarkus is based on the https://narayana.io/docs/project/index.html#d0e16066[Narayana STM] implementation.
...
by a scheduler, all state in memory is lost and the objects start from scratch. But surely you get more
with JTA (and a suitable transactional datastore)? Not quite. There's a trade-off here; we're doing away
with persistent state and the overhead of writing (and sync-ing) to the datastore. So updates to (volatile)
state are fast and you still get the benefits of atomic updates across multiple STM objects, consistency
and isolation in the presence of concurrent threads/users (common in distributed microservices architectures).
Not all stateful applications need to be durable, after all. And as you'll see later, because applications can
optionally start and control transactions, it's possible to build microservices which can undo state changes
...
To use STM in Quarkus applications you must define which objects you would like to be transactional using java
...
Remark: sharing a transaction between multiple services is possible but is currently
