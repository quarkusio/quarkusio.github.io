...
IMPORTANT: The content of this guide has been revised and split into additional topics. Please check the xref:additional-information[Additional Information] section.
...
xref:writing-extensions.adoc#configuration[Writing Your Own Extension] guide.
...
1. (400) xref:system-properties[System properties]
2. (300) xref:environment-variables[Environment variables]
3. (295) xref:env-file[.env] file in the current working directory
4. (260) xref:application-properties-file[Quarkus Application configuration file] in `$PWD/config/application.properties`
5. (250) xref:application-properties-file[Quarkus Application configuration file] `application.properties` in classpath
6. (100) xref:microprofile-config-properties-file[MicroProfile Config configuration file]
...
----
<1> The name `QUARKUS_DATASOURCE_PASSWORD` follows the same conversion rules used for xref:environment-variables[Environment variables].
...
Use xref:property-expressions[expressions] to access it: `${quarkus.uuid}`.
