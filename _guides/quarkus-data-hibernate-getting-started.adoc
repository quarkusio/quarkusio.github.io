...
public class CrowdSourceServiceTest {
    ...
    void shouldInsertAndQueryBooks() {
        crowdSourceService.contributeNewBook("Effective Java", "978-0134685991", BigDecimal.valueOf(45.00));
        crowdSourceService.contributeNewBook("Clean Code", "978-0132350884", BigDecimal.valueOf(40.00));
...
Let's create a `Publisher` entity.
...
Since `Book` is now a managed entity, `insert()` is no longer available. You'll get some compilation errors because `insert()` is not available anymore. Use `persist()` instead:
