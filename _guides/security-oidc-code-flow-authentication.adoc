...
Quarkus OIDC uses the refresh token to refresh the current ID and access tokens as part of its xref:session-management[session management] process.
...
See the xref:db-token-state-manager[Database TokenStateManager] and xref:custom-token-state-manager[Session cookie and custom TokenStateManager] sections of this guide for alternative approaches to storing tokens.
...
For more information, see xref:db-token-state-manager[Database TokenStateManager].
...
For information about the default `TokenStateManager` storing tokens in an encrypted session cookie, see xref:token-state-manager[Session cookie and default TokenStateManager].

For information about the custom Quarkus `TokenStateManager` implementation storing tokens in a database, see xref:db-token-state-manager[Database TokenStateManager].
...
It is similar to xref:back-channel-logout[Back-channel logout] but the logout steps are executed by the user agent, such as the browser, and not in the background by the OIDC provider.
...
xref:user-initiated-logout[User-initiated logout] will log the user out of the OIDC provider.
