...
[NOTE]
====
Once you have downloaded GraalVM, expand the archive and set the `GRAALVM_HOME` variable to this location. For instance:
...
image:native-image-process.png[Creating a native executable]
...
In addition to the regular files, the build also produces an executable: `target/getting-started-1.0-SNAPSHOT-runner`.
...
In the `native` profile of the `pom.xml` file, the following failsafe plugin declaration will run the tests:
...
You'll also find the `src/test/java/org/acme/quickstart/NativeGreetingResourceIT.java` test class in the project:
