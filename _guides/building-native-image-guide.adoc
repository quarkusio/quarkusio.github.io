...
[NOTE]
====
Once you have downloaded GraalVM, expand the archive and set the `GRAALVM_HOME` variable to its location:
...
You can also install GraalVM using https://sdkman.io/[sdkman].
For example, to install the 1.0.0-RC12 version of GraalVM use:

`sdk install java 1.0.0-rc-12-grl`. Then set the `GRAALVM_HOME` variable accordingly:

`export GRAALVM_HOME=$HOME/.sdkman/candidates/java/1.0.0-rc-12-grl`
====
