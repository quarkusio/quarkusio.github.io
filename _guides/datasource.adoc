...
In case you need access Postgresql container based database from external tools, the database name is "default", username is quarkus and password is quarkus. To find out the connecting port number, you may need to run "docker port" to get the locally mapped port number. Alternatively "docker inspect <container_name>" to find the container ip in which case you can just use the standard postgresql port of 5432. Beware that port number/ip is likely to change on each restart.
