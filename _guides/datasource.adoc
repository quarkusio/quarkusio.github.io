...
[NOTE]
====
You can use any JDBC driver in a Quarkus app in JVM mode (see xref:other-databases[Using other databases]).
...
See xref:other-databases[Use a database with no built-in extension or with a different driver] if you want to use a JDBC driver for another database.
...
Read xref:in-memory-databases[Testing with in-memory databases] (below) for suggestions regarding integration testing.
====
...
[TIP]
====
For more information about the JDBC URL format, please refer to the xref:jdbc-url[JDBC url reference section].
====
...
Please refer to the xref:jdbc-configuration[JDBC configuration reference] for all the details about the JDBC configuration knobs.
...
You can override this by setting the `transactions` configuration property - see the xref:configuration-reference[Configuration Reference] below.
