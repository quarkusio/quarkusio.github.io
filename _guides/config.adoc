...
Sometimes we might need to pick specific configuration and use it as an variable in another configuration.

For example we might want to specify different database hostname in certain cases depends how application is packaged: locally or in docker

[source,properties]
----
postgres.host=127.0.0.1                                                 <1>
%docker.postgres.host=postgres                                          <2>
quarkus.datasource.url=jdbc:postgresql://${postgres.host}:5432/postgres <3>
----

<1> By default `postgres.host` pointing on 127.0.0.1 and using when database is running on a same host.
<2> Property `postgres.host` is equals to `prosgres` when application is running with docker profile `mvn quarkus:dev -Dquarkus.profile=docker`.
<3> Final `quarkus.datasource.url` property value will be resulted in honor to `${postgres.host}` current property value, where it is going to be substituted according to used profile.

Compile or start Quarkus application with proper profile according to environment:

[source,shell]
----
./mvnw package -Pnative
./target/my-app-1.0-runner -Dquarkus.profile=docker
----
