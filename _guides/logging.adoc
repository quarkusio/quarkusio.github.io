...
In such cases, you need to use a xref:logging-adapters[logging adapter] to ensure that its log messages are routed to JBoss Log Manager as well.
...
The same flow can be applied with any of the xref:supported-logging-apis[supported logging APIs].
...
Because JBoss Logging is built-in to Quarkus, link:https://quarkus.io/developer-joy/[unified configuration] is provided for all xref:supported-logging-apis[supported logging APIs].
