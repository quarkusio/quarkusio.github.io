...
The solution is located in the `microprofile-fault-tolerance` {quickstarts-tree-url}/microprofile-fault-tolerance[directory].
...
Add a fallback method to `CaffeeResource` and a `@Fallback` annotation to `CoffeeResource#recommendations()` method
as follows:
