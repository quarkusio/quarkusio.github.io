...
a|icon:lock[title=Fixed at build time] [[quarkus-core_quarkus-compose-devservices-startup-timeout]] [.property-path]##link:#quarkus-core_quarkus-compose-devservices-startup-timeout[`+++quarkus.compose.devservices.startup-timeout+++`]##
ifdef::add-copy-button-to-config-props[]
