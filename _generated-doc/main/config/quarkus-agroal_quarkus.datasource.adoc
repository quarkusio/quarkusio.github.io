...
--
a|tooltip:lenient[Allow a thread to acquire multiple connections from the pool without any restriction. This is the default.], tooltip:warn[Log a warning when a thread that already holds a connection tries to acquire another one.], tooltip:strict[Throw an exception when a thread that already holds a connection tries to acquire another one.]
