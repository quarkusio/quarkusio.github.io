...
a| [[quarkus-redis-client_quarkus-redis-username]] [.property-path]##link:#quarkus-redis-client_quarkus-redis-username[`+++quarkus.redis.username+++`]##
ifdef::add-copy-button-to-config-props[]
config_property_copy_button:+++quarkus.redis.username+++[]
endif::add-copy-button-to-config-props[]


`+++quarkus.redis."redis-client-name".username+++`
ifdef::add-copy-button-to-config-props[]
config_property_copy_button:+++quarkus.redis."redis-client-name".username+++[]
endif::add-copy-button-to-config-props[]

[.description]
--
The default username for Redis connections, used for the ACL authentication of Redis 6 and later (`AUTH <username> <password>`).

A username set in a `hosts` URI takes precedence over this property.


ifdef::add-copy-button-to-env-var[]
Environment variable: env_var_with_copy_button:+++QUARKUS_REDIS_USERNAME+++[]
endif::add-copy-button-to-env-var[]
ifndef::add-copy-button-to-env-var[]
Environment variable: `+++QUARKUS_REDIS_USERNAME+++`
endif::add-copy-button-to-env-var[]
--
|string
|
...
If not set and the default proxy configuration is configured (`quarkus.proxy.++*++`), it is used. If set to `none`, no proxy configuration is used. If set to some other value, the configuration from `quarkus.proxy.<name>.++*++` is used. If set but no proxy configuration with that name is found, an exception is thrown at runtime.
