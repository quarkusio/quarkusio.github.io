...
[.description]
--
Configures the source of the authentication credentials. This is typically the database where the credentials have been created. The value defaults to the `authSource` option in the connection string, then to the database specified in the `database` configuration property, then to the database specified in the path portion of the connection string. If none of these are specified, the default value is `admin`. This option is only respected when using the MONGO-CR mechanism (the default).
