...
a|icon:lock[title=Fixed at build time] [[quarkus-core_quarkus-native-enable-fallback-images]] [.property-path]##link:#quarkus-core_quarkus-native-enable-fallback-images[[.line-through]#`+++quarkus.native.enable-fallback-images+++`#]##
ifdef::add-copy-button-to-config-props[]
...
[.description]
--
_This property is deprecated since `4.0`: Fallback images are generally discouraged and are discontinued in GraalVM 25.1 (https://github.com/oracle/graal/pull/12755)._
...
Use slash (`/`) as a path separator on all platforms. Globs must not start with a slash. Globs must not start with a globstar (`++**++`) as wildly general globs that target the entire classpath are rejected.
