...
image:lsp-console-config.webp[Language server configuration]
...
image:lsp-console-traces.webp[Language server traces]
...
image:lsp-settings.webp[Language servers preferences]
...
image:properties-completion.webp[Quarkus properties completion]

Completion is also available for enum values:
image:properties-completion-enum.webp[Quarkus properties enum completion]

=== Inlay hints
Properties using expression values are now resolved as inlay hints:
image:properties-inlay-hint.webp[Resolved property expression as inlay hint]
...
image:reactive-routes.webp[Reactive routes]
...
|===
|image:qute-syntax-coloration-light.webp[Qute Syntax coloration - light theme] |image:qute-syntax-coloration-dark.webp[Qute Syntax coloration - dark theme]
|=== 
...
image:qute-renarde-tags-completion.webp[Renarde tags completion]
...
image:qute-renarde-codelens.webp[Renarde endpoint codelens]

- Completion on the Renarde `uri` and `uriabs` namespaces lists available Renarde controllers:
image:qute-renarde-completion.webp[Renarde uri completion]
...
image:qute-message-bundles.webp[Message bundle support for Qute templates]
...
image:java-validation-graphql-directives.webp[GraphQL directives placement validation]

- Quarkus `BuildItem` subclasses are checked whether they're `final` or `abstract`:
image:java-builditem-validation.webp[BuildItem validation]
...
image:inspections-severity-mapping.webp[Inspection severity mapping]
...
image:exclusions-quickfix.webp[Quick-fix to exclude property from validation]
...
image:exclusions-settings.webp[Inspections settings]


Similarly you can exclude a particular Qute template from validation, or its entire module

image:exclude-qute-validation.webp[Exclude Qute template from validation]

You can then head over to `Preferences | Editor | Inspections | Qute | Templates | Validation`, 

image:qute-validation-settings.webp[Qute validation settings]
