...
Max's second demo covered TamboUI directly. The pitch: Go has https://github.com/charmbracelet/bubbletea[Bubble Tea], Rust has https://github.com/ratatui/ratatui[Ratatui] (which inspired much of TamboUI's design), Python has https://textual.textualize.io/[Textual]. Java had nothing comparable — so Max built it.
...
Ståle's performance team at IBM is already using this pattern. Their new change-detection service for performance regression testing (a system called https://github.com/Hyperfoil/Horreum[Horreum]) grew a CLI layer used by developers to inject test data, trigger analysis, and query results — all while running the same Quarkus service that handles the REST API. CLI and REST share the same service layer, so correctness guarantees on one path cover both.
...
9. **PicoCLI is still a great choice** — if you have a working PicoCLI app and speed is not a concern, there is no imperative to migrate. Aesh is the upgrade path when startup, completion, or REPL capabilities become requirements.
