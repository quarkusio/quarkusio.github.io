...
:topics: observability,opentelemetry, micrometer, observation, jfr, logging, metrics, tracing, monitoring, devservice
:extensions: io.quarkus:quarkus-opentelemetry,io.quarkus:quarkus-micrometer,io.quarkus:quarkus-observation,io.quarkus:quarkus-jfr
...
|xref:telemetry-micrometer-observation.adoc[quarkus-observation]
|
|X
|X
|
...
The xref:telemetry-micrometer-observation.adoc[`quarkus-observation`] extension provides the https://docs.micrometer.io/micrometer/reference/observation.html[Micrometer Observation API], which produces both metrics and traces from a single instrumentation point. It can be combined with any Micrometer registry (Prometheus, OpenTelemetry bridge) and optionally with `quarkus-opentelemetry` for tracing.
...
The xref:telemetry-micrometer-observation.adoc[`quarkus-observation`] extension also provides tracing through the Micrometer Observation API when combined with `quarkus-opentelemetry`. Observations produce OpenTelemetry spans with parent-child relationships and distributed context propagation.
