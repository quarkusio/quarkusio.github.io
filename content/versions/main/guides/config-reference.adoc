...
:categories: core
:categories-path: core.core-config
...
NOTE: By default, a Config Mapping validates that every available configuration name under the mapping prefix is
matched. This prevents unknown configuration properties. This validation can be disabled by setting
...
by the profile name and a dot `.` in the syntax `%[profile-name].config.name` (without the square brackets):
...
accepts a comma-separated list of profile names.
...
The Parent Profile also supports multiple levels of hierarchy. A parent profile can itself have a parent:

.application.properties
[source,properties]
----
quarkus.profile=child
%child.quarkus.config.profile.parent=parent
%parent.quarkus.config.profile.parent=grandparent
----

This activates three profiles with the following lookup priority: `child` overrides `parent`, and `parent` overrides
`grandparent`. If a property is defined in both `child` and `parent`, the `child` value wins.
...
NOTE: Escaping only applies to values that contain an expression segment. In a value with no `$\{...}` segment,
every backslash is treated as a literal character (for example, a Windows path such as `C:\some\path` is used as-is).
Once a value contains an expression segment, backslash becomes an escape character everywhere in that value.

=== Disabling Expansion

Expression expansion may be selectively disabled for a block of code using
`io.smallrye.config.Expressions.withoutExpansion()`. The raw (unexpanded) value is returned for any property looked
up within the block:

[source,java]
----
SmallRyeConfig config = ConfigProvider.getConfig().unwrap(SmallRyeConfig.class);
// returns "https://${remote.host}/" without expanding the expression
...
returns the exact same result. If the same property name exists in both forms within the same configuration source,
the indexed format takes priority. When available in multiple sources, the higher ordinal source wins, like any other
