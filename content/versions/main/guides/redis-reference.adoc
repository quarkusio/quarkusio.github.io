...
:categories: data
:categories-path: data.data-nosql
:numbered:
:sectnums:
...
1. The https://redis.io/try/sandbox/[interactive tutorial] introducing Redis.
...
[source,properties]
----
quarkus.redis.hosts=redis://[[username]:password@]host[:port][/db-number]
----
...
[source,properties]
----
quarkus.redis.hosts=unix://[[username]:password@]/domain/docker.sock[?select=db-number]
----
...
When the Redis server uses https://redis.io/docs/latest/operate/oss_and_stack/management/security/acl/[ACL] users (Redis 6 and later), the username can be set in the `redis://` URL or with the `quarkus.redis.username` property, whose environment variable is `QUARKUS_REDIS_USERNAME`, or `QUARKUS_REDIS_<NAME>_USERNAME` for named clients:

[source,properties]
----
quarkus.redis.hosts=redis://localhost:6379
quarkus.redis.username=my-app
quarkus.redis.password=my-secret
----

Credentials set in the URL take precedence over the `quarkus.redis.username` and `quarkus.redis.password` properties.
