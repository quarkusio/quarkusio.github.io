...
:categories-path: packaging.packaging-native
...
If you prefer relying on the GraalVM infrastructure directly,
you can also create a link:https://www.graalvm.org/{graalvm-docs-version}/reference-manual/native-image/metadata/[reachability-metadata.json] file with a `resources` section defining which resources should be included.
This file should be placed under the `src/main/resources/META-INF/native-image/<group-id>/<artifact-id>` folder.
This way it will be automatically parsed by the native build, without additional configuration.
...
Note that the `reachability-metadata.json` file will be overwritten by Quarkus if placed directly under `src/main/resources/META-INF/native-image/` as Quarkus generates its own configuration files in this directory.
====

[IMPORTANT]
====
Legacy individual configuration files (`resource-config.json`, `reflect-config.json`, `proxy-config.json`, `jni-config.json`, `serialization-config.json`) are not supported by Quarkus. Use the unified `reachability-metadata.json` format.
Quarkus automatically generates `reachability-metadata.json` files for native image compilation.
====
...
The glob patterns use the same syntax as `quarkus.native.resources.includes`.
Here we include all the XML files located under `META-INF` and JSON files located under `config` into the native executable.

[NOTE]
====
Avoid using highly generic patterns like `**/*.xml` without a preceding literal directory. Overly broad patterns starting with a globstar `**` force GraalVM to scan the entire classpath, which can significantly increase build times and bloat the final native executable size. Too generic, globstar beginning globs will trigger a GraalVM warning during the build:

`Warning: Pattern: **/*.xml contains ** without previous literal. This pattern is too generic and therefore can match many resources. Please make the pattern more specific by adding non-generic level before ** level.`
====
...
[IMPORTANT]
====
The legacy `reflect-config.json` file is deprecated.
Use `reachability-metadata.json` with a `reflection` section instead.
====

As an example, in order to register all methods of class `com.acme.MyClass` for reflection, we create `reachability-metadata.json` under `src/main/resources/META-INF/native-image/<group-id>/<artifact-id>`:
...
[NOTE]
====
For more information about the format of this file, see the link:https://www.graalvm.org/{graalvm-docs-version}/reference-manual/native-image/metadata/[GraalVM Reachability Metadata] guide.
====

Place the configuration file under the `src/main/resources/META-INF/native-image/<group-id>/<artifact-id>` folder.
This way it will be automatically parsed by the native build, without additional configuration.
...
[source]
----
com.oracle.svm.core.jdk.UnsupportedFeatureError: Proxy class defined by interfaces [interface org.apache.http.conn.HttpClientConnectionManager, interface org.apache.http.pool.ConnPoolControl, interface com.amazonaws.http.conn.Wrapped] not found. Generating proxy classes at runtime is not supported. Proxy classes need to be defined at image build time by specifying the list of interfaces that they implement.
----

[IMPORTANT]
====
The legacy `proxy-config.json` file and `-H:DynamicProxyConfigurationFiles`/`-H:DynamicProxyConfigurationResources` options are deprecated.
Use `reachability-metadata.json` with a `proxy` section instead.
====

To solve the issue you can create a `reachability-metadata.json` file under the `src/main/resources/META-INF/native-image/<group-id>/<artifact-id>` folder with a `proxy` section.
For more information about the format, see the link:https://www.graalvm.org/{graalvm-docs-version}/reference-manual/native-image/metadata/[GraalVM Reachability Metadata] guide.
...
Alternatively, you may create a `reachability-metadata.json` file as described in <<managing-proxy-classes-app>>.
