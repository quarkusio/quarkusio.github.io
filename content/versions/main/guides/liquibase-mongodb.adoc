...
:categories: data
:categories-path: data.data-migration
...
NOTE: The property `quarkus.liquibase-mongodb.[MongoDB client name].change-log` is the minimal configuration required for the extension to detect a new client setup.
...
<3> Use the Liquibase instance directly
<4> List of applied or not applied liquibase ChangeSets
