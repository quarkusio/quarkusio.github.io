...
:categories-path: serialization.serialization-grpc
...
Quarkus provides two implementations of the gRPC server: gRPC Java (based on Netty) and Vert.x.
Both of them support TLS.

One of the advantages of the Vert.x based server is the ability to use a single server to handle HTTP requests and gRPC requests. This is useful if you want to expose both REST and gRPC endpoints on the same port. This is not possible with the gRPC Java server (using a separate server).
...
NOTE: The separate server mode provides more direct control over Netty-specific settings but requires a dedicated port for gRPC traffic.
...
gRPC Java client only supports the `PEM` format for the trust-store.
