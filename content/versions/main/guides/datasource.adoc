...
:categories: data
:categories-path: data.data-datasources
...
[NOTE]
====
If your application depends on multiple JDBC driver extensions, you will also need to <<datasource-db-kind,select one through configuration>>.
====
...
----
+
Then follow instructions found in <<other-databases,Custom databases and drivers>>.
...
[NOTE]
====
If your application depends on multiple reactive SQL client extensions, you will also need to <<datasource-db-kind,select one through configuration>>.
====
...
When you add both a JDBC extension and a reactive datasource extension for the same database kind, Quarkus creates both JDBC and reactive datasources by default.
...
Notice there is an extra section in the configuration property key.
The syntax is as follows: `quarkus.datasource.[optional name.][datasource property]`.

NOTE: Named datasources must set at least one build-time property so Quarkus can detect them.
Setting `db-kind` is the most common approach.
If the datasource is used by Hibernate ORM, it will be detected automatically;
see <<datasource-detection,Conditions for datasource creation>>.
...
=== Conditions for datasource creation

Quarkus creates a datasource when it detects that one is needed, i.e. when any of the following conditions is met:

* It is requested by another extension (e.g. Hibernate ORM, Flyway, Liquibase).
For example, when Hibernate ORM has entities to manage, it will automatically request the datasource(s) it needs.
* OR at least one build-time property is set for it (e.g. `db-kind`).
* OR no datasource is defined at all: then the default datasource is created.

When a named datasource is not used by any extension that requests it automatically,
you need to set at least one build-time property for it to be created.
Setting <<datasource-db-kind,`db-kind`>> is the most common approach.
This is particularly relevant for named datasources used only through direct injection
(e.g. `@Inject @DataSource("inventory") AgroalDataSource`).

When a datasource is used by Hibernate ORM,
you may not need any configuration for it while developing,
as the database type (<<datasource-db-kind,`db-kind`>>) will be inferred automatically if you only have one JDBC driver in the classpath,
and the database URL, username and password will be filled in automatically by <<dev-services,Dev Services>>.

[[datasource-db-kind]]
=== Applications with multiple datasource extensions

If your application depends on multiple database drivers,
Quarkus may be unable to determine which database kind a given datasource is meant to target.
In that case, the build may fail and prompt you to select a datasource extension through configuration.

[NOTE]
====
// See io.quarkus.datasource.deployment.DataSourceProcessor.resolveDbKinds
Some ambiguities, in particular related to testing, are resolved automatically; see <<in-memory-databases>>.
====

Selecting a datasource extension for the default datasource is achieved with the following configuration property:
...
Below are the `db-kind` values for each of the built-in JDBC drivers:
...
Below are the `db-kind` values for each of the built-in reactive SQL clients:
