...
Additionally, you can adjust the lifespans of the access token and SSO session with the `quarkus.keycloak.devservices.access-token-lifespan` and `quarkus.keycloak.devservices.sso-session-max-lifespan` properties, which are set to 10 minutes by default.
