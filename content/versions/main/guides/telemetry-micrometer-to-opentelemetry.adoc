...
[NOTE]
====
Using a Micrometer registry (such as `quarkus-micrometer-registry-prometheus`) together with OpenTelemetry metrics collects the same measurements twice. Quarkus logs a build-time warning in that case.
See xref:opentelemetry-metrics.adoc#redundant-metrics-warning[Avoiding redundant metrics collection] for the available options, including this extension.
====
...
|===
|Micrometer Meter Java definition |Quarkus Micrometer Prometheus client v0.x output (as seen at `/q/metrics/`) | This bridge OpenTelemetry output name (as seen in the OTLP output) | Related OpenTelemetry Semantic Convention (not applied)
