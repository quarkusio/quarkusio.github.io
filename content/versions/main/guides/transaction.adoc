...
:categories: data
:categories-path: data.data-transactions
...
[NOTE]
====
The transaction context is destroyed once the transaction has completed, so `@PreDestroy` callbacks
of `@TransactionScoped` beans run after the commit or rollback, when the transaction is no longer active.
To run code before the transaction completes, while it is still active, observe the
`@BeforeDestroyed(TransactionScoped.class)` event as shown below,
or use a transactional observer method with `@Observes(during = TransactionPhase.BEFORE_COMPLETION)`.
====
