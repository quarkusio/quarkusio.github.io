...
.Example `{config-file}`
----
%prod.quarkus.datasource.username = hibernate <1>
...
----
<1> xref:datasource.adoc[Configure the datasource] for production, relying on xref:datasource#dev-services[Dev Services] for connection information in tests / dev mode. Setting `db-kind` is only required if there is more than one database extension on the classpath.
...
A persistence unit (and its `SessionFactory`) will be created when Quarkus detects that one is needed.
This happens when any of the following conditions is met:

* JPA entities are detected in the application (the default persistence unit is created automatically).
* Hibernate ORM configuration properties are set (e.g. `quarkus.hibernate-orm.schema-management.strategy`).

Similarly, the datasource that the persistence unit points to is created automatically when it is referenced.
See xref:datasource.adoc#datasource-detection[Datasource detection] for details.
...
They also match the xref:databases-dev-services.adoc[Dev Services] container versions.
...
For instance, the following snippet configures the `schema-management.strategy` property for the default persistence unit:
...
[source,properties]
----
%prod.quarkus.datasource."users".username = hibernate <1>
%prod.quarkus.datasource."users".password = hibernate
%prod.quarkus.datasource."users".jdbc.url = jdbc:postgresql://localhost:26257/users_db

%prod.quarkus.datasource."inventory".username = hibernate <2>
%prod.quarkus.datasource."inventory".password = hibernate
%prod.quarkus.datasource."inventory".jdbc.url = jdbc:postgresql://localhost:26257/inventory_db
...
[[persistence-unit-detection]]
=== Conditions for persistence unit configuration

Quarkus creates a persistence unit when it detects that one is needed, i.e. when any of the following conditions is met:

* It is requested by another extension (e.g. Hibernate Search, Envers).
* OR at least one build-time property is set for it (e.g. `quarkus.hibernate-orm."users".schema-management.strategy`).
* OR at least one entity is attached to it (see <<multiple-persistence-units-attaching-model-classes,below>>).
* OR no persistence unit is defined at all, and the default datasource exists: then the default persistence unit is created.

This applies equally to the default persistence unit and to named persistence units.
You can mix the default persistence unit and named persistence units, or only have a default persistence unit, or only have named persistence units.

xref:datasource.adoc#datasource-detection[Datasources follow similar rules], making most configuration optional <<dev-mode,while developing>>:
for each persistence unit, you mainly need to select a datasource and attach entities.
...
Entity changes are not persisted automatically: you must call `insert()`, `update()`, or `delete()` explicitly.
