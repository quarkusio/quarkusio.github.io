...
Database access is something many applications need, whether it's persisting data or querying tables.
...
We start by mapping our database table to a Java class and defining a repository for it. An _entity_ is a Java class that maps to database tables.
...
<2> `RecordEntity` gives the entity a generated `Long` id field and convenient methods that will be used later. You can use Hibernate's `@Id` and `@GeneratedValue` directly if you prefer, or if you need a custom ID strategy (see the xref:quarkus-data-hibernate.adoc#about-entity-identifiers[reference guide]).
...
<6> `@Query` lets you write an https://docs.hibernate.org/orm/8.0/querylanguage/html_single/#and-sqk[HQL] fragment when you need more control. The `select` and `from` parts are optional: they can be inferred from the return type `List<Book>`.
...
NOTE: The same compile-time checks apply to the query string passed to `@Query`. If you reference a field that doesn't exist or make a syntax error, the build fails. The query language is HQL (Hibernate Query Language); see the https://docs.hibernate.org/orm/8.0/querylanguage/html_single/#and-sqk[HQL and SQL guide] for the full syntax.
...
When you call `insert()`, `update()`, or `delete()`, an operation is instantly triggered against the database, instead of being delayed until the next flush or commit.
...
The previous example of a Discount Service works, but it can be simpler.
...
Quarkus Data Hibernate lets you pick blocking or reactive to match your use case. The same entity model works unchanged with a reactive driver.

First, add the reactive dependencies:

:add-extension-extensions: quarkus-hibernate-reactive,quarkus-hibernate-reactive-panache-common,quarkus-reactive-pg-client
include::{includes}/devtools/extension-add.adoc[]

NOTE: `quarkus-hibernate-reactive-panache-common` is needed because `quarkus-hibernate-reactive` does not yet transitively include the classes required by the annotation processor to generate reactive repository implementations (see https://github.com/quarkusio/quarkus/issues/53871[#53871]).
...
For the full details on reactive sessions, stateless reactive, and combining blocking and reactive code, see the xref:quarkus-data-hibernate.adoc#lets-get-reactive[reference guide].
