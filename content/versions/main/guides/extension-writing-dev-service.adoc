...
public DevServicesResultBuildItem createContainer(MyConfig config) {
    ...
    return DevServicesResultBuildItem.owned()
            ...
            .startable(() -> new MyContainer(
                    ...
                    config.port()))
            .configProvider(
                    s -> Map.of("some-service.base-url",
                            "http://" + s.getConnectionInfo()))
            .build();
}
----
...
s -> Map.of("the-config-key-for-the-base-url",
"http://" + s.getConnectionInfo()))`.
...
`configProvider(Function<T, Map<String, String>>)`::
Provides configuration values that are only known after the service is started, such as mapped ports.
The function receives the started `Startable` and returns a map of config property names to their values.
+
