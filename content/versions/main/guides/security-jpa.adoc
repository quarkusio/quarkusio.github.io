...
:categories: security
:categories-path: security.security-identity
:diataxis-type: howto
:topics: security,identity-providers,sql,database,jpa,jdbc
:extensions: io.quarkus:quarkus-security-jpa,io.quarkus:quarkus-security-jpa-reactive
...
[NOTE]
====
The identity provider is available for both persistence stacks:

* `quarkus-security-jpa` for applications that use Hibernate ORM.
* `quarkus-security-jpa-reactive` for applications that use Hibernate Reactive.

The entity annotations and the configuration described in this guide are the same for both extensions.
====
...
public class User extends PanacheEntity {
     ...
     */
    public static void add(String username, char[] password, String role) { <6>
        User user = new User();
...
[TIP]
====
To quickly create a hashed password, use `String BcryptUtil.bcryptHash(char[] password)`, which defaults to creating a random salt and hashing in ten iterations.
