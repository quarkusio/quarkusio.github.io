...
=== Limiting extension catalog to an offering

NOTE: This feature was introduced in Quarkus 3.26.0 and will be evolving in the upcoming versions.

As described in link:{quarkus-home-url}/guides/extension-metadata#extension-offerings[Extension offering metadata], extensions may be associated with a one or more offerings delivered and supported by a dedicated team or an organization.
...
- registry.quarkus.io
----
<1> limits the extension catalog provided by `registry.acme.org` to the extensions that make up the `acme-cloud` offering.

NOTE: If an offering is not configured, the complete catalog of extensions returned by the registry will be provided to the user.
