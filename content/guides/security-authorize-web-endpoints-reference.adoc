...
[source,java]
----
@PermissionsAllowed("see\\:detail") <1>
void matched() { }

@PermissionsAllowed("see:detail") <2>
void notMatched() { }

@PermissionChecker("see\\:detail")
boolean canAccess(SecurityIdentity identity) {
    return identity.hasRole("admin");
}
----
<1> Matched by the permission checker because both raw annotation values are `see\\:detail`.
<2> Not matched because the raw value `see:detail` differs from the checker value `see\\:detail`.
