...
:categories: cloud
:categories-path: cloud.cloud-deployment
...
An important thing to note about the `Deployment` (or `StatefulSet`) is that it uses `yourDockerUsername/test-quarkus-app:1.0.0-SNAPSHOT` as the container image of the `Pod`.
...
To simplify things, Quarkus provides a single-step alternative:
