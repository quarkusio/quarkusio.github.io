...
:categories-path: miscellaneous.miscellaneous-templating
...
IMPORTANT: The `{#eval}` section helper is not registered automatically. It can be registered manually using `engine.addSectionHelper(new EvalSectionHelper.Factory())`. See <<engine-customization>> for more details.
...
NOTE: When using `quarkus-rest-qute` or `quarkus-resteasy-qute` the content negotiation is performed automatically.
...
If using <<rest_integration,templates in Jakarta REST resources>>, you can rely on the following convention:
...
** IMPORTANT: For security reasons, the `str:eval` namespace resolver is _not_ registered automatically. Evaluating an untrusted string as a template may result in a server-side template injection. If you need this functionality then you have to register the `io.quarkus.qute.StrEvalNamespaceResolver` manually, e.g. via <<engine-customization,`@Observes EngineBuilder`>>:
+
[source,java]
----
import io.quarkus.qute.EngineBuilder;
...
[[rest_integration]]
=== [[resteasy_integration]] REST Integration
...
NOTE: When using <<rest_integration,`quarkus-rest-qute`>> (or `quarkus-resteasy-qute`) the `locale` attribute is derived from the `Accept-Language` header if not set by a user.
