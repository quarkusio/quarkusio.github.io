...
Then, create a `.gcloudignore` file to tell gcloud which files should not be uploaded for Cloud Build,
without it, it defaults to `.gitignore` that usually exclude the target directory where your packaged application has been created.
