...
Navigate to your Spring Boot project to be migrated and launch your agent.

image::01-project-to-migrate.png[alt=Application to migrate, align=left, width=60%]

WARNING: Configure the agent authentication mode using the API Key and/or subscription according to the provider before to launch it !
...
The agent detects from the installed Skills the one to be used according to the following text "Migrate this Spring Boot" that the agent is using to match the proper skill when reading the skill's description.
