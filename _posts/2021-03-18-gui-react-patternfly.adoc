...
While this approach has the benefit of independent container scaling as well as indenpendent lifecycle it may sometimes be over engineered for small applications.
...
Import the Patternfly CSS to the React project in */webapp/index.js*
...
In /webapp/src create a new folder called *components*, inside a file called *particles.js* and generate a Patternfly table by looping over a list of particle objects: 
...
Open your browser to http://localhost:8080 to see you table again, now running from a single optimized jar.
