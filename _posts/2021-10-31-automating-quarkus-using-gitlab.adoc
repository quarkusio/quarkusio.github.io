...
The most empowering feature of the pipeline, is merge requests can deploy any number services, which can be interacted with, to isolated preview environments

image::gitlab/develop-pipeline.png[Review Pipeline,align="center"]

Developers can go beyond just automated testing and can spot-test individual services or even run a full suite of services if needed (this is especially useful during a combined UX and Service feature).
