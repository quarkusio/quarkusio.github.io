...
TIP: Quarkus extensions are what make the whole process so powerful. Individually, they are easy enough to setup and add value to each service but, when combined with an automated end-to-end pipeline, their simplicity and flexibility starts to shine.  Similar to the DevPanel available from `quarkus:dev` you can use the extensions together to get a complete overview of your whole platform from development to deployment.
