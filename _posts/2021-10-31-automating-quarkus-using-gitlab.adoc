...
== The Challenge
In moving to https://quarkus.io/[Quarkus^] productivity quickly began to outpace our CI/CD process. We needed a better way to leverage microservices and cloud-native approaches, deploy faster and more frequently, and finally involve everyone throughout the entire process.
...
This is our evolving blueprint of how we leverage https://quarkus.io/[Quarkus^], especially the extensions, with https://about.gitlab.com/[GitLab^] and tie everything together into one cohesive ecosystem that is accessible by everyone.
...
The *cluster* project centralizes anything touching the Kubernetes cluster. It is configured as a GitLab https://docs.gitlab.com/ee/user/clusters/management_project_template.html[cluster-management^] project so that we can include managing other cluster-wide services like Elastic, Nginx, or any Helm charts.
...
Each Quarkus project is structured as a _family of services_ that are individually worked on and deployed. Each service then uses triggers to notify the cluster when its ready to be deployed or cleaned up.
...
Using Quarkus we elminated having to maintain separate Helm charts for each service. We even have Quarkus services that interact directly with GitLab for specific tooling and metrics we maintain.
...
Weaving Quarkus extensions together we are able to get a complete picture of our environments, from development to deployment and monitoring - it has made developing, debugging, and supporting microservices easier. 

Teams can now use GitLab to view exposed logs, trace services, see errors, etc.
...
TIP: Previously, deployments to higher environments was tedious but doable. Developers were not involved in this process. Since moving to Quarkus, developers are now responsible for developing _and_ deploying (limitations apply). 

Using GitLab, we settled on https://docs.gitlab.com/ee/ci/pipelines/merge_request_pipelines.html[merge requests pipelines^] to be the catalyst that drove our process.
...
Deploying _preview_ services lets us go beyond automated testing. We can immediately spot-test what is being worked on or even run a full suite of services if needed (this is especially useful during a combined UX and multiple service feature).
