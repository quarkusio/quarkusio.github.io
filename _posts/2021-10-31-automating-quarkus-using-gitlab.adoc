...
Each Quarkus project is structured as a _family of services_ that are individually worked with and deployed. Each service uses triggers to notify the cluster when its ready to be deployed or cleaned up.
...
Using Quarkus we elminated having to maintain separate Helm charts for each service. We even have Quarkus services that interact directly with GitLab for specific tooling and metrics we use.
