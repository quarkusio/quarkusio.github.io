...
- Build the executable as a native image or FastJar
...
Once a container has been built and the manifests uploaded, that package can be referenced anytime (or we can rollback to any previous version if needed).

TIP: We _only_ push the container and upload the manifests to AWS S3. This lets us accumulate, over time if needed, all changes into a single package that will be slightly augmented right before deployment. 
