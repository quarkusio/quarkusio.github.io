...
== Quarkus Extensions
The Quarkus extensions that play important roles in our release process are:app-name: 
...
From within GitLab we expose these services to the developers and operations teams.  This minimizes the need to have to access AWS or Kubernetes directly and provides a single location for most needs.

Prometheus Metrics are visible from GitLab.

image::gitlab/prom.png[Prometheus Metrics,align="center"]

Annotating the Kubernetes manifests with GitLab specific annotations lets us track their health right in GitLab.

image::gitlab/view-pods.png[Pod Health,align="center"]

Using Elastic we can view the logs for the namespace or pods in GitLab.

image::gitlab/view-logs.png[Cluster Logs,align="center"]

Using the Quarkus Sentry Logging Extension we can view our errors right inside GitLab.

image::gitlab/view-sentry.png[Sentry Errors,align="center"]

Using the Quarkus OpenTracing Extension we can trace our services inside GitLab.

image::gitlab/view-jaeger1.png[Jaeger Query,align="center"]

image::gitlab/view-jaeger2.png[Jaeger Details,align="center"]

Using Quarkus Jacoco we generate coverage for each project.

image::gitlab/coverage.png[Coverage Report,align="center"]

For any branch you can access that specific version of Swagger. If you are in a merge-request it can be accessed right from the the ticket.
