...
TIP: Quarkus extensions are what make the whole process so powerful. Individually, they are easy enough to setup and add value to each service but, when combined into an automated pipeline, their simplicity and flexibility starts to shine.  Similar to the DevPanel available from `quarkus:dev` you can use them to get a complete overview of your whole platform during development and at runtime.
