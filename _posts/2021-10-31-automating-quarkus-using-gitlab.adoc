...
== GitLab Projects
The structure we settled on, for our various Quarkus services and GitLab projects, separates the services, cluster operations, and templates into individual projects. This allows us to introduce changes to existing configurations or apply new ones with minimal effort and maximum flexibility.

The `cluster` project centralizes all of our cluster pipelines and orchestrates the Quarkus service deployments using downstream triggers. It is setup as a cluster managed project in GitLab so that we manage more general cluster services, such as Helm charts deployments too.

image::gitlab/cluster-project.png[GitLab Cluster Project,align="center",width="292"]

The `templates` project contains the various building blocks used to support end-to-end building, testing, and deployment of Quarkus services as well as other stacks. It serves as the foundation in which all other projects are built on top of.

image::gitlab/templates-project.png[GitLab Templates Project,align="center",width="292"]

Quarkus services are structured either using a MonoRepository, when we can group similar services together, or a more traditional per-service repository. Through the templates, we can compose and accomodate how they are treated regardless of approach.

By templatizing the Quarkus service pipeline specifically, we can choose to mix and match approaches and still maintain a consistent approach to pipeline execution. An important part of those templates is having the flexibility to direct the context where a service resides - either at a folder level or the root of the project.

=== MonoRepository
The _Platform_ project below is a MonoRepo. All of its services are contained under a `services` folder but handled by pipelines as if they were each unique projects.

image::gitlab/mono-repo.png[GitLab MonoRepository Project,align="center",width="292"]

=== Per-Service Repository
_Services_ is a collection of projects and shows the per-service approach. Each project is a single specific service rather than a family of services. 
...
Leveraging these extensions has added significant value to our process. Aside from centralizing the most common needs of developers, and the cluster, it also saves time, increases visibility, and reduces the learning curve for all of our teams.
