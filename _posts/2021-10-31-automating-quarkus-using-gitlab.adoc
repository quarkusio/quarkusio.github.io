...
Our development pipelines, which is comprised Quarkus-based services, is heavily based on the use of merge-requests in GitLab. Our goal was to provide as much visibility and functionality within the merge-requests so that developers could easily accomplish their day-to-day work without the barriers associated with microservice architectures.

From the merge-request we:

- Run functional tests 
- Run integration tests (Mongo, Redis)
- Run E2E tests (AWS, external services)
- Generate code coverage and quality reports
- Provide access to Swagger per branch
- Deploy one or more services into isolated namespaces

image::gitlab/merge-request.png[Merge Request,align="center"]

=== Merge-Request Pipeline
A typical pipeline for a merge-request looks like the below image.

image::gitlab/develop-pipeline.png[Review Pipeline,align="center"]

=== Downstream Quarkus Pipeline
...
Once the tag has been created the release pipeline runs and by default builds and deploys each of the Quarkus services in parallel to the next environment.

image::gitlab/release-pipeline.png[Deploy to Staging,align="center"]

In the end, the formal release is created automically and the version is bumped to the next version.
