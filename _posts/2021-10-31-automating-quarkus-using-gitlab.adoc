...
- https://quarkus.io/guides/tests-with-coverage[Quarkus Jacoco Extension^] to provide coverage https://docs.gitlab.com/ee/ci/pipelines/settings.html#add-test-coverage-results-to-a-merge-request[results^] and https://docs.gitlab.com/ee/user/project/merge_requests/test_coverage_visualization.html[reports^]
...
TIP: Quarkus extensions are what make the whole process so powerful. Individually, they are easy enough to setup and add value to each service but, when made an integral part of an automated process, their simplicity and flexibility really start to shine. By weaving extensions together you can get a complete picture of your environment: all phases from development to deployment and beyond.
