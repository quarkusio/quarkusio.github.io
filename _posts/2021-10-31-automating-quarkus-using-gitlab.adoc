...
== GitLab Projects
The project structure we settled on, for Quarkus services and in GitLab, separates the various services, cluster operations, and templates into individual projects. This gives us the most flexibility when making changes to existing configurations or applying new ones.

The `cluster` project centralizes all of our cluster pipelines and orchestrates all of the services using deploy and cleanup downstream triggers.

image::gitlab/cluster-project.png[GitLab Cluster Project,align="center",width="292"]

The `templates` project contains the various building blocks used to support end-to-end building, testing, and deployment of Quarkus services as well as other stacks. It serves as a foundation in which all other projects are built atop of.

image::gitlab/templates-project.png[GitLab Templates Project,align="center",width="292"]

Quarkus services are structured using either a MonoRepository, when we can group similar services, or traditional per-service repositories. Regardless of the approach, both are handled the same way.
...
When using a MonoRepository approach, each service is tracked separately so that only what was changed gets processed during a pipeline.

== Quarkus Extensions
We leverage the functionality provided by Quarkus extensions extensively in our projects and since GitLab offers hooks for many of the extensions out-of-the-box, integrating them with GitLab has been simple and straight-forward.  The value from this, is we consolidate what would normally require access to different toolsets in one place.

The https://quarkus.io/guides/deploying-to-kubernetes[Quarkus Kubernetes^] and https://quarkus.io/guides/kubernetes-config[Quarkus Kubernetes Config^] extensions are the most important ones we use as they drive most of the pipelines.

Other Quarkus extensions used throughout our process and tied into GitLab are:
...
Often times all that is needed to integrate an extensions functionality with GitLab is as simple as adding the extension to the project, configuring its properties, and then pointing to the GitLab to the service.
