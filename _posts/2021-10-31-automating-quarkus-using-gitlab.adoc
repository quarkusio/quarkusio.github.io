...
The _templates_ project contains the various building blocks (templates) used to support end-to-end building, testing, and deployment of Quarkus services as well as other stacks. This lets us use a single project to serve as the source for all Gitlab-ci job scripts which allows easy maintainence and flexability.  Each Quarkus service uses these default templated jobs so each individual Quarkus services's Gitlab-ci script is very small.
