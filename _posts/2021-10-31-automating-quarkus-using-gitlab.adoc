...
In the examples above, the _Platform_ project is a MonoRepo, all its services are contained within a `services` folder in that project even through they are each deployed separately.  The second image, _Services_, shows the per-service approach and each project is a single specific service rather than a family of services. 

By templatizing the Quarkus service pipeline, we can choose to mix and match approaches and always have a consistent setup.
