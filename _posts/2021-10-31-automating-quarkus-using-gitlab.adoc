...
Most importantly, each merge request can deploy any number of services that can interacted with. This lets developers go beyond just automated testing and do anything from a spot-testing a single service to running a full suite of services (if needed). This all happens within the isolation of their own branch and, when done, gets cleaned up automatically.
