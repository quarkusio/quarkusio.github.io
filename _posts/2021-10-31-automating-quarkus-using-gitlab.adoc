...
The end result of tightly integrating our GitLab process with our Quarkus-based platform is that not only has our process become greatly streamlined but, we also are able to provide a one-stop shop for most of the relevant tools, logs, and monitoring by all of our teams.  
...
Teams can use GitLab instead of having to interact with different external tools and applications (except in advanced scenarios).

=== Interactive API Endpoints
Swagger can be viewed and interacted with directly in GitLab. This allows for quick and easy spot testing during the development or validating endpoints post-deployment.

image::gitlab/swagger.png[Swagger API,align="center"]

=== Errors and Warnings
Sentry is hooked into each project so that specific errors and warnings can be found quickly and easily without having to dig through the logs.  We can even create or resolve tickets directly from the issue.  
