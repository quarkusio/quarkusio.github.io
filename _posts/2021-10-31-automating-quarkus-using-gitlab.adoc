...
Merge requests perform these actions on every push:
...
Most importantly, each merge request can deploy any number of services, that can be interacted with, to its own isolated environment.

image::gitlab/develop-pipeline.png[Review Pipeline,align="center"]

Developers can go beyond just automated testing and spot-test a single service or even run a full suite of services if needed (this is especially useful during a combined UX and Service feature).
...
The key parts of the pipeline, and where the magic happens, are the downstream jobs used for handling Quarkus services and cluster deployments.

=== Building Quarkus Services
The downstream Quarkus build is a specialized pipeline specific to building, testing, and containerizing (only) just Quarkus services. 
