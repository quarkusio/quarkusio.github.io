...
Last week we became aware that the OpenJDK team in Java 15 have disabled so called *biased locking* in the Java virtual machine. This is change from previous years and could *potentially* have a negative impact on Java application's performance.
