...
Although reducing memory footprint is not one of Leyden's primary goals, it may occur as a side effect.
...
- Startup and warmup time improvements are significant, but not as dramatic as with GraalVM/Mandrel native image.
...
We fully expect this packaging to evolve over time as we gather feedback from real-world usage.
...
TODO Georgios Before the conclusion, I want to add a section that mentions that we are actively monitoring the developments of Leyden via the premain branch.
