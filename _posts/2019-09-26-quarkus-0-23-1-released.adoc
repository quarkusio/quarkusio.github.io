...
synopsis: We just released Quarkus 0.23.1 which is our largest release so far. The most important change is the introduction of the first bricks of our new HTTP layer.
author: gsmet
---
...
0.23.1 is the first step of this big change: our servlet implementation is now making use of Vert.x under the hood. Note that this is only the first change of many to come as, in our next version, RESTEasy will be able to avoid servlets entirely and leverage Vert.x directly.
