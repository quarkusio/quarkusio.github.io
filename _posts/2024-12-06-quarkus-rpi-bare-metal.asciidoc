...
A Raspberry Pi is not infrequently extended with various sensors and pheripherals.
...
In other words, usually you cannot build the Quarkus app on your machine and run it on my Raspberry Pi.
...
You'll also need the GraalVM/https://github.com/graalvm/mandrel/releases[Mandrel] native-image, or Docker/Podman to build the native image in the container.
...
You can also try to build the native image in a container using an ARM-based image.
...
<4> `-Dquarkus.native.builder-image=quay.io/quarkus/ubi-quarkus-mandrel-builder-image:23.1.5.0-Final-java21-arm6` specifies an ARM-based image that should be used to build the native image.
