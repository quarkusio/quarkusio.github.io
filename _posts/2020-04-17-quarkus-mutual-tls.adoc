---
layout: post
title: 'Quarkus - Makes Mutual TLS Easy.'
date:   2020-04-17 00:00 +0100
author: mkouba
synopsis: Quarkus - Mutual TLS Authentication.
---
...
It is an additional security layer which verifies the identity of the client.

== How implement it?

The best approach to implement Mutual TLS between two services has to come from the infrastructure, for instance, Service Mesh, to achieve a standard and secure way of communication and to avoid that each application implements their own solution. However, you are not always working on cutting edge environment.

Let's have a look at how to implement mTLS with Quarkus.
 
== Bootstrapping.

Bootstrapping Hello server application,
...
NOTE: For this example, the `client.keystore` used as _truststore_ for the server application and in the same way the `server.keystore` for the client application.
...
To understand if the response is coming from the server application, you will update the `org.acme.server.mtls.GreetingResource` class.
...
At this point, the server application is ready to accomplish Mutual TLS. Let's configure the client.

[source]
----
code quarkus-client-mtls/
----
